...
```{r, fig.cap = "Elicited and fitted probabilities using the normal distribution"}
if(showplots){
  curve(pnorm(x, mean = mu, sd = sqrt(sigsq)), from = lx, to = ux, xlab = "x",
        ylab = "P(X <= x)", ylim = c(0, 1))
  points(fit$vals[expert, ], fit$probs[expert, ], pch = 16, col = "red")
...
```{r, fig.cap = "Elicited and fitted probabilities using the $t$-distribution"}
if(showplots){
  curve(pt((x - m)/s, df = tdf), from = lx, to = ux, xlab = "x",
        ylab = "P(X <= x)", ylim = c(0, 1))
  points(fit$vals[expert, ], fit$probs[expert, ], pch = 16, col = "red")
...
<hr>
#### Log normal (lower limit: `r fit$limits[expert, 1]`)
...
```{r, fig.cap = "Elicited and fitted probabilities using the log normal distribution"}
if(showplots & !is.na(fit$ssq[4])){
  curve(plnorm(x - fit$limits[1, expert],
               meanlog = mu, 
               sdlog = sqrt(sigsq)), from = lx, to = ux, xlab = "x",
        ylab = "P(X <= x)", ylim = c(0, 1))
  points(fit$vals[expert, ], fit$probs[expert, ], pch = 16, col = "red")
...
<hr>
#### Gamma  (lower limit: `r fit$limits[expert, 1]`)
...
```{r, fig.cap = "Elicited and fitted probabilities using the gamma distribution"}
if(showplots & !is.na(fit$ssq[3])){
  curve(pgamma(x - fit$limits[1, expert],
               shape = shape, 
               rate = rate), from = lx, to = ux, xlab = "x",
        ylab = "P(X <= x)", ylim = c(0, 1))
  points(fit$vals[expert, ], fit$probs[expert, ], pch = 16, col = "red")
...
<hr>
#### Log Student$-t$  (lower limit: `r fit$limits[expert, 1]`)
...
```{r, fig.cap = "Elicited and fitted probabilities using the log Student-$t$ distribution"}
if(showplots & !is.na(fit$ssq[5])){
  curve(pt((log(x) - m)/s - fit$limits[1, expert],
               df = tdf), from = lx, to = ux, xlab = "x",
        ylab = "P(X <= x)", ylim = c(0, 1))
  points(fit$vals[expert, ], fit$probs[expert, ], pch = 16, col = "red")
...
<hr>
#### Beta  (lower limit: `r fit$limits[expert, 1]`, upper limit: `r fit$limits[expert, 2]`)
```{r, echo = FALSE}
if(fit$limits[expert, 1] == 0)
  x <- paste0("x")
if(fit$limits[expert, 1] > 0)
...
```{r, fig.cap = "Elicited and fitted probabilities using the beta distribution"}
if(showplots & !is.na(fit$ssq[6])){
  curve(pbeta((x - fit$limits[1, expert])/r,
               shape1 = shape1, 
               shape2 = shape2), from = lx, to = ux, xlab = "x",
        ylab = "P(X <= x)", ylim = c(0, 1))
  points(fit$vals[expert, ], fit$probs[expert, ], pch = 16, col = "red")
